...
- **화면 오버레이**: 작업 중인 세션마다 러너 한 마리가 화면 하단을 가로질러 달립니다.
  클릭은 통과되므로 작업을 방해하지 않습니다. (메뉴에서 끌 수 있음)
...
- **입력 요청 감지**: Claude가 권한 승인이나 질문(환경변수 값, 키 입력 등)으로 멈춰서
  당신의 응답을 기다리면, 그 세션의 동물이 제자리에서 크게 **수직 점프**하며 ❗ 표시가
  붙습니다. 러너가 화면 꼭대기에 있어 뛸 공간이 없으면 반대로 아래로 파고듭니다.
  메뉴바 아이콘도 ❗로 바뀌고 알림음이 울립니다.
...
`claude` / `gemini` / `codex` CLI 프로세스를 PID(세션) 단위로 추적하고, 세션별
최근 몇 초간의 CPU 사용률(자식 프로세스 포함 — 긴 빌드·테스트 실행도 "작업 중"으로
잡힘)로 "실제 작업 중"과 "입력 대기 중"을 구분합니다. 데몬·pty 헬퍼 같은 상주 보조
프로세스는 제외하며, 세션의 작업 디렉토리는 libproc(`proc_pidinfo`)으로 읽습니다.
