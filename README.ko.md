...
벽을 밀어보지만 나아가지 못합니다. 그리고 터미널에서 답하는 순간 벽은 바로
사라집니다 — 알림이 아니라 세션의 실제 대화 기록을 따라가기 때문입니다.

**🖱️ 동물을 클릭하면 지금 어디까지 왔는지 보여줍니다.** 마우스를 올리고 클릭하면
어느 프로젝트에서 무엇을 시켰는지, 그리고 방금 뭐라고 답하고 있는지까지 보여줍니다.
벽 앞에 멈춰 있다면 **실제로 무엇을 물어보고 있는지** — 질문 원문이나 허가를
기다리는 도구 호출(`Bash — git push …`) — 를 보여줍니다. 벽돌도 클릭됩니다.

**🐾 메뉴바가 곧 출석부입니다.** 상태 표시줄에 달리는 세션 수만큼 동물이
나란히 표시되고, 막힌 세션은 🧱🐕처럼 벽을 앞세웁니다. 메뉴에서 세션 항목에
마우스를 올리면 러너를 클릭했을 때와 같은 상세 정보가 나옵니다.

**📉 높이 = 남은 Claude 사용량.** Claude 사용량 한도는 5시간 블록으로 갱신됩니다.
블록이 새로 시작되면 러너가 화면 위쪽에서 달리고, 사용할수록 점점 아래로 내려오며
서서히 흐려집니다. 윈도우를 다 쓰면 러너는 작은 묘비가 되어 새 블록이 시작될
때까지 멈춰 섭니다. 바닥에 붙어서 달리고 있다면 곧 리셋이니 슬슬 마무리할
때입니다.
...
단위로 추적합니다. 인터랙티브 터미널 세션만 셉니다 — 데몬·pty 헬퍼 같은 상주 보조
프로세스, `claude auth status`나 `claude -p …` 같은 일회성 호출, 다른 세션의 도구가
띄운 CLI는 모두 제외하고, 새 PID는 두 번의 폴링을 버텨야 화면에 나타납니다.
데브 서버가 몇 초마다 `gemini auth status`를 실행해도 유령 러너가 쏟아지지 않는
이유입니다. 작업 디렉터리는 libproc(`proc_pidinfo`)으로 읽어 폴링마다 별도
프로세스를 띄우지 않습니다.
...
완료 알림은 무시합니다. 훅 보고는 대화 기록과 일치하는 동안만 신뢰합니다 — Esc로
턴을 중단하면 `Stop` 훅이 발생하지 않으므로 대화 기록이 3분간 조용한 "작업 중"
보고는 신뢰하지 않고, 권한 승인 역시 아무 훅도 발생시키지 않으므로 대화 기록이
알림 시점을 지나쳐 진행되면 "입력 대기" 보고를 즉시 폐기합니다. 훅에는 세션의
실제 ID도 담겨 있어서, 한 프로젝트에서 세션 여러 개를 돌려도 각자 자기 로그
파일을 정확히 찾습니다.

클릭 패널의 상세 정보도 대화 기록에서 읽습니다 — 가장 최근의 어시스턴트 텍스트가
"최신 응답"이고, 로그 끝에 결과 없이 남은 `tool_use`가 대기 중인 질문
(`AskUserQuestion`이면 질문 원문 그대로) 또는 허가를 기다리는 도구 호출입니다.
